...
finished_players = {'ed': [384713574, 150], 'ed1': [384713574,
                                                    3500]}  # hashtable for players whos done first step (set point of first location)
                ...
                async def generate_leaderboard():
                    ...
                    if 3 <= len(finished_players) <= 5:  # condition for limit players count in leader_board (3-5)
                        ...
                        for idx, pl in enumerate(
                                dict(sorted(finished_players.items(), key=lambda x: x[1], reverse=True)), start=1):
