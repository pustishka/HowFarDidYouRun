...
finished_players = {'Katrin_Gri': [384713574, 150.0], 'ed1': [384713574,
                                                    3500.0]}  # hashtable for players whos done first step (set point of first location)
...
# main handler for all process in bot
...
async def handle_location(message: types.Message):
    ...
    async def calculate_distance():
        ...
        if username in players and race_done_mark:  # condition for access to second step (calculate distance from second long and lat)
            ...
            async def generate_leaderboard():
                ...
                if 3 <= len(finished_players) <= 5:  # condition for limit players count in leader_board (3-5)
                    ...
                    for idx, pl in enumerate(dict(sorted(finished_players.items(), key=lambda x: x[1][1], reverse=True)), start=1):
                        # enumerate count and username to leader_board in nondecreasing order
                        leader_board += f'{idx}. {pl} - {finished_players[pl][1]} метра\n'
                    # print(sorted(finished_players.items(), key=lambda x: x[1][1], reverse=True))
            ...
            await generate_leaderboard()  # call function
        else:
...
if __name__ == '__main__':
    # t = threading.Thread(target=main_func)  # create thread
